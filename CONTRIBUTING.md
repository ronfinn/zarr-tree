...
`zarr-tree` is a personal project with one maintainer. This file is how it is
developed: what you need to build it, what the quality gate is, and which
design constraints a change is expected to preserve. There is no contribution
process — work happens directly on `master` in small commits.
...
4. Add a `CHANGELOG.md` entry under `[Unreleased]` if a user would notice it.
5. Run the quality gate above.
6. Read `git diff` before committing. It catches more than the linters do.
7. Commit to `master` and push.

Commit messages use a short prefix — `feat:`, `fix:`, `docs:`, `test:`,
`chore:` — followed by an imperative summary and a paragraph on why. They carry
no AI attribution: no `Co-Authored-By:` trailer for an AI, no Anthropic
address, no session URL, no "Generated by" line.

Releasing is a version bump in `Cargo.toml`, a dated heading in
`CHANGELOG.md`, a commit and a tag. Tags are never moved once pushed.
...
If you have found a bug, a plain
[GitHub issue](https://github.com/ronfinn/zarr-tree/issues) with the command,
what it printed, and the smallest metadata document that reproduces it is all
that is needed. Leave out credentials, signed URLs and dataset content.
